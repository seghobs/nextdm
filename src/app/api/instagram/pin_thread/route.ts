...
export async function POST(request: NextRequest) {
  try {
    ...
    const { threadFbid, pin, cookies, headers, data } = body;

    // Validate parameters
    if (!threadFbid || pin === undefined) {
      return NextResponse.json({
        success: false,
        error: 'Missing required parameters: threadFbid or pin'
      }, { status: 400 });
    }
    ...
    const headersToSend: Record<string, string> = {
      ...
      'x-fb-friendly-name': 'useIGDPinThreadMutation',
      'referer': `https://www.instagram.com/direct/inbox/`,
    };

    // Construct variables object for useIGDPinThreadMutation
    const variablesObj = {
    ...
    const postDataFields = {
      ...DEFAULT_DATA,
      ...customData,
      'fb_api_req_friendly_name': 'useIGDPinThreadMutation',
      'variables': JSON.stringify(variablesObj),
      'doc_id': '26491513390471063', // Pin thread mutation doc_id
    };
    ...
    console.log(`Proxying pin/unpin thread (${pin ? 'pin' : 'unpin'}) for thread_fbid: ${threadFbid}...`);
    ...
    if (status === 301 || status === 302 || status === 307 || status === 308) {
      console.warn('[PinThread-API] Instagram redirected the request (likely login required).');
      return NextResponse.json({
        success: false,
    ...
    if (!instagramResponse.ok) {
      console.error(`Instagram API returned non-OK status on pin thread: ${status}`, responseText);
      return NextResponse.json({
        success: false,
        status,
        error: `Instagram API returned status ${status} on pin thread`,
    ...
    try {
      ...
      if (jsonResponse?.error === 1357004) {
        console.warn('[PinThread-API] Mismatched web tokens detected (Error 1357004). Attempting self-healing...');
        
        const { scrapeTokens } = await import('@/lib/token-scraper');
        const { fbDtsg, lsd } = await scrapeTokens(cookieHeaderStr);
        
        if (fbDtsg || lsd) {
          console.log('[PinThread-API] Successfully scraped fresh tokens. Retrying request...');
      ...
      if (jsonResponse.errors) {
        return NextResponse.json({
          success: false,
          error: 'Instagram returned GraphQL errors on pin thread',
      ...
      return NextResponse.json({
        success: false,
        error: 'Invalid JSON response from Instagram on pin thread',
    ...
    console.error('Error in pin thread proxy route:', error);
    return NextResponse.json({
      success: false,
