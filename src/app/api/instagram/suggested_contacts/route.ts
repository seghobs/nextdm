...
export async function POST(request: NextRequest) {
  try {
    ...
    const headersToSend: Record<string, string> = {
      ...
      'x-fb-friendly-name': 'IGDOmniPickerNullStateListQuery',
      'referer': `https://www.instagram.com/direct/inbox/`,
    };

    // Construct variables object for IGDOmniPickerNullStateListQuery
    const variablesObj = {
      input: {
        count_per_page: 20,
    ...
    const postDataFields = {
      ...
      'doc_id': '27657376130569675', // Suggested contacts search doc_id
    };
    ...
    console.log(`Proxying IGDOmniPickerNullStateListQuery for suggested contacts...`);
    ...
    if (status === 301 || status === 302 || status === 307 || status === 308) {
      console.warn('[SuggestedContacts-API] Instagram redirected the request (likely login required).');
    ...
    if (!instagramResponse.ok) {
      console.error(`Instagram API returned non-OK status on suggested contacts: ${status}`, responseText);
      return NextResponse.json({
        success: false,
        status,
        error: `Instagram API returned status ${status} on suggested contacts`,
    ...
    try {
      ...
      if (jsonResponse?.error === 1357004) {
        console.warn('[SuggestedContacts-API] Mismatched web tokens detected (Error 1357004). Attempting self-healing...');
        ...
        if (fbDtsg || lsd) {
          console.log('[SuggestedContacts-API] Successfully scraped fresh tokens. Retrying request...');
          ...
          if (retryResponse.ok) {
            ...
            const items = retryJson.data?.get_paginated_share_sheet_ranked_items?.ranked_items || [];
      ...
      if (jsonResponse.errors) {
        return NextResponse.json({
          success: false,
          error: 'Instagram returned GraphQL errors on suggested contacts',
          details: jsonResponse.errors,
          cookies: updatedCookies
        }, { status: 200 });
      }

      const items = jsonResponse.data?.get_paginated_share_sheet_ranked_items?.ranked_items || [];
      ...
      return NextResponse.json({
        success: false,
        error: 'Invalid JSON response from Instagram on suggested contacts',
    ...
    console.error('Error in suggested contacts proxy route:', error);
    return NextResponse.json({
      success: false,
